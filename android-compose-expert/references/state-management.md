...
`collectAsStateWithLifecycle()` is in `androidx.lifecycle:lifecycle-runtime-compose`. It collects only when the composable is in a `STARTED` state, avoiding memory leaks and unnecessary background work.
...
**Always prefer `collectAsStateWithLifecycle()` over `collectAsState()`** when collecting from `StateFlow` or `SharedFlow` in Android Compose. Reserve `collectAsState` for non-lifecycle-aware contexts only.
