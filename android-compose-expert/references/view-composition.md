...
Every UI composable must have a `@Preview` for rapid visual validation. See
`references/preview.md` for the full guide covering `PreviewParameterProvider`,
`PreviewTheme` wrappers, multi-device strategies, and screenshot testing reuse.
...
@Composable
fun UserCardPreview() {
    PreviewTheme { UserCard(user = User("Alice")) }
}
